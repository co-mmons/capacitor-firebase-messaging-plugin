...
export namespace FirebaseMessagingPlugin {

    /**
     * Open permission settings for current app. On iOS it will open settings related to system notifications,
     * on android it will open "about app" view, where the user will be able to grant system notifications.
     */
    export function openNotificationsPermissionSettings() {
     ...
     * @return On Android only `NotificationsPermissionState.granted` and `NotificationsPermissionState.denied`.
     */
    export async function notificationsPermissionState(): Promise<NotificationsPermissionState> {
        let result = await Plugins.FirebaseMessaging.notificationsPermissionState();
        return result.state;
    }
    ...
    export async function subscribeToTopic(topic: string): Promise<void> {
        await Plugins.FirebaseMessaging.subscribeToTopic({topic: topic});
    }
    ...
    export async function unsubscribeFromTopic(topic: string): Promise<void> {
        await Plugins.FirebaseMessaging.unsubscribeFromTopic({topic: topic});
    }
     ...
     * @param listener 
     */
    export function addListener(eventName: "messageReceived", listener: (message: RemoteMessage) => void): PluginListenerHandle {
